...
#define MAX_Y -(MIN_Y)
#define DEFAULT_STEP ((MAX_X)-(MIN_X))
#define TICK_STEP 1.0f
#define TICK_SIZE 5.0f
#define RESOLUTION 100.0f
...
int main(void)
{
    ...
    while (!WindowShouldClose()) {
        ...
        const Cartesian cartesian = {
            ...
            .x_scale = camera.zoom*screen_width/DEFAULT_STEP,
            .y_scale = camera.zoom*screen_height/DEFAULT_STEP
        };
        ...
        BeginDrawing();
            ...
            float min_x = MIN_X/camera.zoom + camera.offset.x;
            float max_x = MAX_X/camera.zoom + camera.offset.x;
            float min_y = MIN_Y/camera.zoom + camera.offset.y;
            float max_y = MAX_Y/camera.zoom + camera.offset.y;
